...
def price_agent_node(state: Blackboard) -> dict:
    ...
    for product in products[:5]:
        ...
        if amazon_data:
            ...
            price_score = max(0, 100 - (price_inr / 50000) * 100)
    ...
    current_confidence = state.get("confidence", {})
    avg_confidence     = (
        sum(v.get("price_score", 0) / 100 for v in findings["products_analyzed"].values())
        / max(len(findings["products_analyzed"]), 1)
    )
    ...
    return {
        ...
        "supervisor_notes": [f"[Price Agent] Priced {len(findings['products_analyzed'])} products | confidence={avg_confidence:.2f}"],
    }
...
def ranker_agent_node(state: Blackboard) -> dict:
    ...
    for product_name in set(list(review_data.keys()) + list(price_data.keys())):
        quality_score      = review_data.get(product_name, {}).get("rating", 0)
        price_score        = price_data.get(product_name,  {}).get("price_score", 0)
        availability_score = 100 if price_data.get(product_name, {}).get("availability", False) else 0
        ...
        ranked_products.append({
            ...
            "review_data":        review_data.get(product_name, {}),
            "price_data":         price_data.get(product_name, {}),
        })
    ...
    result = {
        "ranked": [
            {
                ...
                "price":        round(p["price_score"], 1),
                "availability": round(p["availability_score"], 1),
                "price_inr":    p["price_data"].get("price_inr", "N/A"),
                "rating":       p["price_data"].get("rating", "N/A"),
        ...
        "winner": {
            ...
            "why":            f"Highest combined score ({winner['combined_score']:.1f}/100)" if winner else "No products found",
            "reasoning":      (
                f"Quality {winner['quality_score']:.0f}% × 0.5 + "
                f"Price {winner['price_score']:.0f}% × 0.3 + "
                f"Availability {winner['availability_score']:.0f}% × 0.2 = "
        ...
        "methodology":      "Combined scoring: Quality (reviews) 50% + Price (Amazon.in) 30% + Availability 20%",
        "synthesis_notes":  f"Evaluated {len(ranked_products)} products | Winner selected by weighted scoring algorithm",
    }
...
def parallel_agents_node(state: Blackboard) -> dict:
    ...
    review_result = review_agent_node(state)
    price_result  = price_agent_node(state)
...
def find_best_product(query: str) -> dict:
    ...
    for r in ranked[:10]:
        print(
            f"      #{r.get('rank','?')} {r.get('name','?')[:30]:<30} — "
            f"Score: {r.get('score','?')}/100  "
            f"(Quality: {r.get('quality','?')}, Price: {r.get('price','?')}, "
